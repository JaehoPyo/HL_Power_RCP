...
begin
  ...
  begin
    if  Filename[i] = '.' then
    begin
      msg := copy(Filename,1,i-1) + '.x'+
             copy(Filename,i+1,length(Filename)-i );
      break;
    end;
  end;

  if Msg = '' then LogFileOld := Filename + FormatDateTime('YYMMDDhhnn', Now)
              else LogFileOld := Msg + FormatDateTime('YYMMDDhhnn', Now);

  // Read File 처리
  AssignFile(R, LogFile);       Reset(R);       ReadLn(R, Msg);

  // Write File  처리
  AssignFile(F, LogFileOld);    Rewrite(F);     Writeln(F, Msg );
